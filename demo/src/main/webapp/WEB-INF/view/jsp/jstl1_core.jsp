...
<body style="margin:40px;">
	...
	<table border="1">
		<tr><th>아이디</th><th>이름</th><th>도시</th><th>국가</th></tr>
	<c:forEach var="member" items="${memberArray}"> <!--memberList/memberArray에 있는 데이터 하나하나를 member라는 변수에 넣는다-->
		<tr>
			<td>${member.id}</td>
			<td>${member.name}</td>
	...
	<c:forEach var="member" items="${memberList}" varStatus="loop">
		<tr>
			...
			<c:if test="${member.id mod 2 eq 1 }"> <%--ㅡ${member.id % 2 == 1} --%>
				<span style="color: blue;">${member.id}</span> <%--jstl에는 if문에 else가 없다. 고로 if문을 두번 써야한다 --%>
			</c:if>
			...
			<c:if test="${not loop.first and not loop.last }"><%--또는 not (loop.first or loop.last) --%>
				${member.name}
			</c:if>
			</td>
			<td> <%--주소가 한국이면 배경색 orange, 미국이면  skyblue--%>
